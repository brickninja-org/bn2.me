...
import { OAuth2RequestHandlerProps } from 'src/app/api/(oauth)/request';
import { validateRequest } from '../authorize/validate';
import { AuthorizationRequestExpiration, createAuthorizationRequest } from 'src/app/(authorize)/authorize/helper';
...
export async function handleParRequest({ params, requestAuthorization, headers, url }: OAuth2RequestHandlerProps): Promise<PushedAuthorizationRequestResponse> {
  ...
  if(dpopHeader) {
    ...
    if (params.dpop_jkt !== undefined && params.dpop_jkt !== dpop.jkt) {
      throw new OAuth2Error(OAuth2ErrorCode.invalid_dpop_proof, { description: 'dpop_jkt and the DPoP header must match if using both' });
    }
