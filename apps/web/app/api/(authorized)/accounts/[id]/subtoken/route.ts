...
import { SubtokenResponse, Scope } from '@bn2me/client';
...
export const GET = withAuthorization<RouteProps<{ id: string }>>({ oneOf: Bn2Scopes })(
  async (authorization: Authorization, _, { params }) => {
    ...
    // get required bn2 permissions from authorization scope
    const requiredPermissions = (authorization.scope as Scope[])
      .filter((scope) => Bn2Scopes.includes(scope))
    ...
    try {
      ...
      return NextResponse.json({ error: true, error_message: 'The Brickset API returned an error when creating the subtoken' }, { status: 500 });
    }
...
async function createSubtoken(apiToken: { id: string, token: string }, requiredPermissions: string[]): Promise<SubtokenResponse> {
  ...
  try {
    ...
    throw new Error('The Brickset API returned an error when creating the subtoken', { cause: e });
  }
